...
def calc_lc(mej, vej, beta, kappa_r, kappa_cut=False):
    ...
    Mdiff = (4.0*np.pi*beta*(M0)**(1./3.)*(v0*c*t**2.)/(3.0*kappa_r))**(3./4.)
    Mdiff[Mdiff > M0] = M0
    Mdiff = Mdiff/Msun
    ...
    for j in range(tprec-1):
        ...
        else: kappa_correction[:] = 1.0

        # update all mass layers at time j
    ...
    return tdays, Ltotm*1e40, F_lam, Tobs, Rphoto
